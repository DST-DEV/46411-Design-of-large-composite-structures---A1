...
class TsaiHill(FailureCriterion):
    ...
    def failure_index(self, stress: np.ndarray) -> float:
        ...
        s1, s2, t12 = stress
        s_hat_1 = self.s_hat_1t if s1 >= 0 else self.s_hat_1c
        s_hat_2 = self.s_hat_2t if s2 >= 0 else self.s_hat_2c
    ...
    def failure_envelope(self, n_points: int = 400, t12: float = 0.0
        ...
        s_hat_1 = np.where(s1 >= 0, self.s_hat_1t, self.s_hat_1c)

        shear_term = (t12 / self.t_hat_12) ** 2
        ...
        # Solve assuming σ2 ≥ 0
        s2_t_pos, s2_t_neg = solve_branch(self.s_hat_2t)

        # Solve assuming σ2 < 0
        s2_c_pos, s2_c_neg = solve_branch(self.s_hat_2c)
...
if __name__ == "__main__":
    ...
    tsai_hill = TsaiHill(s_hat_1t=1080e6, s_hat_1c=620e6,
    ...
    for i, (s1_i, s2_i, t6_i) in enumerate(zip(s1, s2, t6)):
        if tsai_hill.failure_index((s1_i, s2_i, t6_i)) < 1:
